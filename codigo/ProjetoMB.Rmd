...
Fazemos cada um dos fatores como modelos separado

```{r}
# Student Ethics
model.ethics <- 'StudentEthics =~ ET12 + ET13'
fit.ethics <- bcfa(model.ethics, data = df, std.lv = TRUE)
summary(fit.ethics, standardized = TRUE, rsquare = TRUE)

# Motivation
model.motivation <- 'Motivation =~ Mot5 + Mot8 + Mot11'
fit.motivation <- bcfa(model.motivation, data = df, std.lv = TRUE)
summary(fit.motivation, standardized = TRUE, rsquare = TRUE)

# Self-Efficacy
model.selfefficacy <- 'SelfEfficacy =~ SE1 + SE2 + SE3 + SE4 + SE5 + SE6'
fit.selfefficacy <- bcfa(model.selfefficacy, data = df, std.lv = TRUE)
summary(fit.selfefficacy, standardized = TRUE, rsquare = TRUE)

# Resilience
model.resilience <- 'Resilience =~ R2 + R5 + R6'
...
# Team Strain
model.teamstrain <- 'TeamStrain =~ TS10 + TS11 + TS12 + TS13 + TS14 + TS15 + TS16 + TS17'
...
model.coopclass <- 'CooperativeClassroomEnvironment =~ CCE1 + CCE3 + CCE4 + CCE5 + CCE8 + CCE9 + CCE10 + CCE11'
fit.coopclass <- bcfa(model.coopclass, data = df, std.lv = TRUE)
summary(fit.coopclass, standardized = TRUE, rsquare = TRUE)
```
