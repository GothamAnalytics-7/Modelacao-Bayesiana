...
```{r}
correr_single_CFAs <- FALSE
correr_multiple_CFA <- FALSE
correr_SEM <- TRUE
correr_SEM_priors <- TRUE
```
...
Vamos primeiro experimentar com todos as variáveis dos itens que são citados no artigo. Após isso, experimentamos com aquelas que são mencionadas no artigo para fazer os construtos.
...
#### Estimação com FA criadas seguidas
...
```{r}
inspect(model.sem, what = "std")$lambda
```
...
model.sem_CFAOUR <- bcfa(model.sem_CFAOUR, data=df_FAour, std.lv=TRUE, n.chains = n_chains,
                 burnin=burn_in, sample=sample_estimate, target = "stan")
```

```{r}
summary(model.sem_CFAOUR, standardized=TRUE, rsquare=TRUE)
```

```{r}
semPaths(model.sem_CFAOUR,
         what = "std",      
         layout = "tree", 
         edge.label.cex = 1.0,
         sizeMan = 10,       
...
Vai ser igual à análise de cima
...
model.sem_meanOUR <- bcfa(model.sem_meanOUR, data=df_FAour, std.lv=TRUE, n.chains = n_chains,
                 burnin=burn_in, sample=sample_estimate, target = "stan")
```

```{r}
summary(model.sem_meanOUR, standardized=TRUE, rsquare=TRUE)
```

```{r}
semPaths(model.sem_meanOUR,
         what = "std",      
         layout = "tree", 
         edge.label.cex = 1.0,
         sizeMan = 10,       
...
```{r}
curve(dgamma(x, 25, 50), from=0, to=1.2, main="(Ñ) Prior para Lambda (loadings)", col = "green", ylab="Densidade")
...
curve(dlnorm(x, meanlog = -0.7, sdlog = 0.3),
      ...
      main = "(Ñ) Distribuição Lognormal(-0.7, 0.3) para Lambda (loadings)")
...
curve(dnorm(x, 0, 0.25), from=-1, to=1,col = "blue", main="Prior para Beta (regressão)", ylab="Densidade")
...
```{r}
blavCompare(fit.sem_priors, model.sem)
```
